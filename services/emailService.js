...
// Create transporter with environment variables (using GoDaddy SMTP with port 465 SSL)
const transporter = nodemailer.createTransport({
  ...
  authMethod: 'LOGIN', // Try LOGIN authentication
  tls: {
...
// Alternative GoDaddy SMTP configuration (port 587 STARTTLS)
const createAlternativeGoDaddyTransporter = () => {
  return nodemailer.createTransport({
    ...
    authMethod: 'PLAIN', // Try PLAIN authentication
    tls: {
      rejectUnauthorized: false,
      ciphers: 'SSLv3'
    },
...
const createAlternativeTransporter = () => {
  return nodemailer.createTransport({
    ...
    authMethod: 'CRAM-MD5', // Try CRAM-MD5 authentication
    tls: {
      rejectUnauthorized: false
    },
    debug: true,
...
const testEmailConfig = async () => {
  try {
    ...
    // Try alternative GoDaddy SMTP (port 465 SSL with PLAIN auth)
    console.log('🔧 Testing alternative GoDaddy SMTP (port 465 SSL with PLAIN auth)...');
    try {
      const altGoDaddyTransporter = createAlternativeGoDaddyTransporter();
      await altGoDaddyTransporter.verify();
      console.log('✅ Alternative GoDaddy SMTP (port 465 SSL with PLAIN auth) is working');
      return true;
    } catch (altGmailError) {
      console.error('❌ Alternative GoDaddy SMTP (port 465 SSL with PLAIN auth) failed:', altGmailError.message);
    }
    
    // Try alternative GoDaddy SMTP server (port 587)
    console.log('🔧 Testing alternative GoDaddy SMTP server (port 587)...');
    try {
      const altGoDaddyTransporter = createAlternativeGoDaddyServer();
      await altGoDaddyTransporter.verify();
      console.log('✅ Alternative GoDaddy SMTP server (port 587) is working');
      return true;
    } catch (altGoDaddyError) {
      console.error('❌ Alternative GoDaddy SMTP server (port 587) failed:', altGoDaddyError.message);
    }
    
    // Try alternative GoDaddy SMTP server (smtp.secureserver.net)
    console.log('🔧 Testing alternative GoDaddy SMTP server (smtp.secureserver.net)...');
    try {
      const altGoDaddyServer = createAlternativeGoDaddyServer();
      await altGoDaddyServer.verify();
      console.log('✅ Alternative GoDaddy SMTP server (smtp.secureserver.net) is working');
      return true;
    } catch (altGoDaddyServerError) {
      console.error('❌ Alternative GoDaddy SMTP server (smtp.secureserver.net) failed:', altGoDaddyServerError.message);
    }
